...
csr = x509.CertificateSigningRequestBuilder().subject_name(x509.Name([
    ...
    x509.NameAttribute(NameOID.COMMON_NAME, "messenger.kfc"),
])).add_extension(
    x509.SubjectAlternativeName(
        [x509.DNSName("messenger.kfc"), x509.DNSName("localhost"),
         x509.IPAddress(ipaddress.IPv4Address("192.168.50.100"))]),
...
certificate = x509.CertificateBuilder().subject_name(
    ...
    x509.SubjectAlternativeName([x509.DNSName("messenger.kfc"), x509.DNSName("localhost"),
                                 x509.IPAddress(ipaddress.IPv4Address("192.168.50.100"))]),
    critical=False).sign(private_key, hashes.SHA256(), default_backend())
